...
countdown x = countdown' "Ready!" x

countdown' result 0 = result ++ " Liftoff!"
countdown' result x = countdown' (result ++ (show x) ++ "... ") (x-1)
...
smallestDivisor' x y
  | rem x y == 0 = y
  | otherwise = smallestDivisor' x (y + 1)
